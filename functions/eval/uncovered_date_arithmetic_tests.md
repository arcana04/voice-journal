...
| gap-05-literal-midnight-reminder | "Our cat has this thing where she scratches at the door around midnight wanting to go out, so let's set an actual reminder for midnight tonight so I don't sleep through it and she doesn't wake up the whole house." | 1 task, reminder_at = today/tonight at literal 00:00 (midnight), NOT null | run 2026-09-23 20:55: "Set reminder for cat at midnight", due_date Sep 24 (Thu), notification fired at **Sep 23 22:55** (2 hours after creation — the all-day-task fallback default) | ❌ | **Confirmed real regression, NOT fixed yet — needs a design decision, not a blind code change.** The model almost certainly emitted `reminder_at` ending in `T00:00:00` correctly, but the server-side safety net (added to fix the unrelated "date-only task defaults to fake midnight" bug) unconditionally nulls out ANY reminder_at ending in `T00:00:00`, with no way to tell a genuine literal-midnight request apart from "time unknown." Once nulled, the task fell back to the all-day-task "2 hours after creation" default notification — arguably more confusing than a plain null, since it fires at a seemingly-arbitrary time (22:55) that has no relationship to what the user asked for. Options going forward: (a) add an explicit boolean field (e.g. `is_explicit_midnight`) the model sets only when the speaker literally said "midnight"/"12am", and have the safety net check it before nulling — same pattern as the other structured fields, but adds prompt complexity for a narrow case; (b) accept this as a known, documented edge-case limitation given how rarely someone means literal midnight via voice memo. Flagging for the user to decide rather than picking one unilaterally. |
