...
interface IdeaAngleResult {
  title: string;
  description: string;
}

interface BrainstormIdeaResult {
  angles: IdeaAngleResult[];
}

function buildIdeaBrainstormSystemPrompt(locale: Locale): string {
  if (locale === "en") {
    return `You are a creative brainstorming partner helping the user deepen and expand on one of their own saved ideas.

You will be given the idea's title and content below. Propose exactly 3 genuinely different angles for taking this idea further — not 3 minor variations of the same thought, but 3 distinct directions (e.g. a different audience, a different execution approach, a related but bigger opportunity, a cheap way to test/validate it, a way to combine it with something else). Respond with a JSON object in this exact shape:

{
  "angles": [
    { "title": "A short punchy title for this angle (under ~8 words)", "description": "1-2 concrete sentences developing this specific angle, grounded in the idea as given — not generic startup advice." },
    { "title": "...", "description": "..." },
    { "title": "...", "description": "..." }
  ]
}

Always return exactly 3 angles. Write in English. Output ONLY the JSON object, no extra commentary.`;
  }

  return `あなたはユーザー自身が保存した1つのアイデアを、一緒に深掘りしてくれるブレインストーミングの相棒です。

以下にそのアイデアのタイトルと内容を渡します。このアイデアを発展させるための、本当に異なる3つの切り口を提案してください——同じ発想の微妙なバリエーションを3つ並べるのではなく、例えば「対象を変える」「実現方法を変える」「関連するもっと大きな機会」「安く試す方法」「他の何かと組み合わせる」のような、はっきり異なる方向性にしてください。必ず以下の形のJSONオブジェクトで出力してください：

{
  "angles": [
    { "title": "この切り口を表す短くキャッチーなタイトル(15文字程度)", "description": "この切り口を具体的に発展させる1〜2文。渡されたアイデアの内容に根ざしたものにし、一般論的なアドバイスにはしないこと。" },
    { "title": "...", "description": "..." },
    { "title": "...", "description": "..." }
  ]
}

必ず3つの切り口を返してください。日本語で書いてください。JSONオブジェクトのみを出力し、余計な説明文は含めないでください。`;
}

async function brainstormIdeaAngles(
  apiKey: string,
  title: string,
  content: string,
  locale: Locale
): Promise<BrainstormIdeaResult> {
  const systemPrompt = buildIdeaBrainstormSystemPrompt(locale);
  const userContent =
    locale === "en"
      ? `[Idea title]\n${title || "(none)"}\n\n[Idea content]\n${content}`
      : `【アイデアのタイトル】\n${title || "（なし）"}\n\n【アイデアの内容】\n${content}`;
  ...
  return JSON.parse(data.choices[0].message.content) as BrainstormIdeaResult;
}

interface BrainstormIdeaRequest {
  title?: string;
  content?: string;
  locale?: string;
}
...
/**
 * Proプラン限定機能。アイデア画面から、保存済みの1件のアイデアを渡して
 * 深掘りする3つの切り口をAIに提案させる。相談機能（askKnowledgeBase）と
 * 同じくPro専用のシングルショットAI呼び出しで、日次の録音クォータは消費しない。
 */
export const brainstormIdea = onCall(
  {
    secrets: [openAiApiKey],
    timeoutSeconds: 60,
    memory: "256MiB",
  ...
  async (request) => {
    const { title, content, locale } = (request.data ?? {}) as BrainstormIdeaRequest;
    const loc = normalizeLocale(locale);
    ...
    try {
      const apiKey = openAiApiKey.value();
      const result = await brainstormIdeaAngles(
        apiKey,
        (title ?? "").trim(),
        content.trim(),
        loc
      );
      const angles = (result.angles ?? []).slice(0, 3).map((a) => ({
        title: a.title ?? "",
      ...
      logger.error("brainstormIdea unexpected error", err);
      const message = err instanceof Error ? err.message : String(err);
      throw new HttpsError("unavailable", MESSAGES[loc].unexpectedError(message));
    }
  }
