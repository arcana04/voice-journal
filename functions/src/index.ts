...
【時刻付きリマインダー】
tasksの中に「15時に」「明日の朝9時」「夜7時に病院」のように"時刻"まで明言されているものがあれば、上記の今日の日付とユーザーの現地時間を基準に実際の日時を計算し、reminder_at に "YYYY-MM-DDTHH:mm:00"（24時間表記、秒は00固定）の形式で入れてください。日付の指定がなく時刻のみの場合は今日の日付を使い、その時刻がすでに過ぎていれば翌日の日付にしてください。時刻の明言が無い場合（日付や「午前中」「そのうち」のような曖昧な言い回ししか無い場合）は reminder_at は null にしてください。**日付だけが分かっていて時刻が分からないからといって、0時(00:00)や仮の時刻をreminder_atに入れないでください**——「明日歯医者に行く」のように日付(明日)しか無く時刻の言及が無い場合、reminder_atは文字列"00:00"ではなく必ずJSONのnullにしてください。0時は「時刻が不明」を表す値ではなく、実際に真夜中を意味してしまいます。
...

{
  ...
  "summary": "全体の1行要約",
  "tasks": [
    {"title": "タスク内容", "due_hint": "期限の元の言い回し（なければnull）", "due_date": "YYYY-MM-DD（推測できなければnull。recurrenceを使う場合もnullでよい）", "reminder_at": "YYYY-MM-DDTHH:mm:00（時刻の明言が無ければnull）", "reminder_end_at": "YYYY-MM-DDTHH:mm:00（終了時刻の明言が無ければnull）", "relative_offset_minutes": 録音時点からの相対時間（「30分後」「3時間後」等）の場合のみ、今から何分後かを表す整数。それ以外は省略するかnull, "span_days": 複数日にまたがる終日の予定の場合のみ、開始日を含めた合計日数の整数（例:「金曜から日曜まで」なら3。最終日の夜が日付をまたいで終了時刻がある場合はまたいだ先の日も含める）。単日の予定では省略するかnull, "recurrence": {"weekdays": ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"のうち繰り返す曜日を英語3文字表記で], "start_date": "YYYY-MM-DD（繰り返し開始日）", "end_date": "YYYY-MM-DD（繰り返し終了日。終了時期の言及が無ければ、自分で計算しようとせずnullにする——アプリ側のコードが妥当な既定値を補う）", "interval_weeks": 1以上の整数（省略時1=毎週。「隔週」等なら2）} … 繰り返しパターンでない通常のタスクではこのフィールド自体を省略するかnullにする, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sunのいずれか", "weeks_ahead": 0以上の整数（0=直近の該当日、1=その1週間後、2=その2週間後...）, "days_before": 0以上の整数（省略時0。別タスクの締切を「その前に」で借りる場合のみ1以上）, "anchor_title": days_beforeを1以上にした場合のみ——その基準イベント（例:「いとこの結婚式」）自体が、話者自身の行動動詞を伴わない単なる事実の言い方（「結婚式は土曜日だ」等）で、このタスクとは別の独立したタスクとしては抽出しないと判断した場合に、その基準イベントを短い名詞句で入れる（例:"Cousin's wedding"）。基準イベント自体を既に別のタスクとして抽出済みの場合は省略するかnullにする。} … 曜日名を軸に期限が語られた場合のみ使用し、それ以外は省略するかnullにする, "due_month": {"months_ahead": 今月を0とした月数（来月なら1、3ヶ月後なら3）, "day": 話者が具体的な日にちを指定した場合のみ1〜31の整数（省略時は今日と同じ日にちを使う）} … 「来月」「3ヶ月後」「1日までに」のような月単位の相対表現で期限が語られた場合のみ使用し、それ以外は省略するかnullにする}
  ],
  "notes": [
...
[Timed reminders]
If a task explicitly states a time (e.g. "at 3pm", "tomorrow morning at 9", "7pm at the clinic"), compute the actual date/time relative to today's date and the user's local time above, and put it in reminder_at as "YYYY-MM-DDTHH:mm:00" (24-hour time, seconds fixed at 00). If only a time is given with no date, use today's date, and if that time has already passed today, use tomorrow's date instead. If no explicit time is stated (only a date, or a vague phrase like "in the morning" or "sometime"), set reminder_at to null. **Knowing the date but not the time of day is never a reason to put midnight (00:00) in reminder_at** — for example, "go to the dentist tomorrow" has a date (tomorrow) but no time-of-day mention at all, so reminder_at must be the JSON value null, not a string ending in "T00:00:00". 00:00 does not mean "time unknown" — it means literally midnight, which is almost never what the speaker meant.
...

{
  ...
  "tasks": [
    {"title": "task content, in English", "due_hint": "original due-date phrase (or null)", "due_date": "YYYY-MM-DD (or null if it can't be inferred; can also be null when using recurrence)", "reminder_at": "YYYY-MM-DDTHH:mm:00 (or null if no explicit time)", "reminder_end_at": "YYYY-MM-DDTHH:mm:00 (or null if no explicit end time)", "relative_offset_minutes": only for a relative time from the recording moment ("in 30 minutes", "in 3 hours", etc.) — an integer number of minutes from now; omit or null otherwise, "span_days": only for a multi-day all-day event — an integer total number of days including the start day (e.g. "Friday through Sunday" = 3; include the extra day too if an end time is given and the final night crosses past midnight), "recurrence": {"weekdays": [array of repeating weekdays, using English 3-letter abbreviations from "Mon","Tue","Wed","Thu","Fri","Sat","Sun"], "start_date": "YYYY-MM-DD (recurrence start)", "end_date": "YYYY-MM-DD (recurrence end; if no end was ever mentioned, set this to null rather than computing one yourself — the app's own code fills in a sensible default)", "interval_weeks": integer ≥1, defaults to 1 = every matching week; use 2 for "biweekly"/"every other week"} — omit this field or set it to null for an ordinary non-recurring task, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sun", "weeks_ahead": non-negative integer (0 = the nearest occurrence, 1 = one week after that, 2 = two weeks after, ...), "days_before": non-negative integer, defaults to 0; only use 1+ when borrowing another task's deadline for a "before then" relationship, "anchor_title": only when days_before is 1+ — if the anchor event itself (e.g. "my cousin's wedding") is a plain factual statement with no first-person action verb, and you decided NOT to also extract it as its own separate task, put a short noun phrase naming that anchor event here instead (e.g. "Cousin's wedding"); omit or set to null if the anchor event is already being extracted as its own task.} — only use this when the due date is centered on a weekday name; omit or null otherwise, "due_month": {"months_ahead": integer, number of months ahead of the current month (0 = this month, 1 = next month, 3 = three months from now), "day": integer 1-31, only if the speaker named a specific day-of-month (omit to reuse today's day-of-month)} — only use this when the due date is a month-based relative expression like "next month"/"in three months"/"by the 1st"; omit or null otherwise}
  ],
  "notes": [
...

{
  ...
  "tasks": [
    {"title": "contenido de la tarea, en español", "due_hint": "frase original de la fecha límite (o null)", "due_date": "YYYY-MM-DD (o null si no se puede inferir; también puede ser null si usas recurrence)", "reminder_at": "YYYY-MM-DDTHH:mm:00 (o null si no hay hora explícita)", "reminder_end_at": "YYYY-MM-DDTHH:mm:00 (o null si no hay hora de fin explícita)", "relative_offset_minutes": solo para una hora relativa al momento de la grabación ("dentro de 30 minutos", "dentro de 3 horas", etc.) — un entero con los minutos desde ahora; omite o pon null en caso contrario, "span_days": solo para un evento de todo el día que abarca varios días — un entero con el total de días incluyendo el día de inicio (p. ej. "viernes hasta el domingo" = 3; incluye también el día extra si hay una hora de fin y la última noche cruza la medianoche), "recurrence": {"weekdays": [array de días que se repiten, usando abreviaturas en inglés de 3 letras de "Mon","Tue","Wed","Thu","Fri","Sat","Sun"], "start_date": "YYYY-MM-DD (inicio de la recurrencia)", "end_date": "YYYY-MM-DD (fin de la recurrencia; si nunca se mencionó un final, pon null en vez de calcular uno tú mismo — el propio código de la app rellena un valor por defecto razonable)", "interval_weeks": entero ≥1, por defecto 1 = cada semana coincidente; usa 2 para "biweekly"/"cada dos semanas"} — omite este campo o ponlo en null para una tarea normal no recurrente, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sun", "weeks_ahead": entero no negativo (0 = la ocurrencia más cercana, 1 = una semana después, 2 = dos semanas después, ...), "days_before": entero no negativo, por defecto 0; usa 1+ solo al tomar prestada la fecha límite de otra tarea para una relación "antes de eso", "anchor_title": solo cuando days_before es 1+ — si el propio evento ancla (p. ej. "la boda de mi primo") es una simple afirmación fáctica sin verbo de acción en primera persona, y decidiste NO extraerlo también como su propia tarea separada, pon aquí un sustantivo corto que nombre ese evento ancla (p. ej. "Cousin's wedding"); omite o pon null si el evento ancla ya se está extrayendo como su propia tarea.} — úsalo solo cuando la fecha límite gire en torno a un nombre de día de la semana; omite o pon null en caso contrario, "due_month": {"months_ahead": entero, número de meses por delante del mes actual (0 = este mes, 1 = el mes que viene, 3 = dentro de tres meses), "day": entero 1-31, solo si el hablante mencionó un día concreto del mes (omite para reutilizar el día del mes de hoy)} — úsalo solo cuando la fecha límite sea una expresión relativa basada en meses como "el mes que viene"/"dentro de tres meses"/"para el día 1"; omite o pon null en caso contrario}
  ],
  "notes": [
...
[시각이 있는 리마인더]
tasks 중 "오후 3시에", "내일 아침 9시", "저녁 7시 병원" 처럼 시각까지 명시된 것이 있으면, 위의 오늘 날짜와 사용자의 현지 시간을 기준으로 실제 날짜/시각을 계산해 reminder_at에 "YYYY-MM-DDTHH:mm:00" 형식(24시간제, 초는 00 고정)으로 넣으세요. 날짜 없이 시각만 있으면 오늘 날짜를 사용하고, 그 시각이 오늘 이미 지났다면 내일 날짜를 사용하세요. 명시적인 시각이 없는 경우(날짜만 있거나 "오전 중", "언젠가" 같은 모호한 표현만 있는 경우)는 reminder_at을 null로 두세요. **날짜는 알지만 시각을 모른다고 해서 reminder_at에 자정(00:00)을 넣지 마세요** — 예를 들어 "내일 치과에 간다"는 날짜(내일)는 있지만 시간대 언급이 전혀 없으므로, reminder_at은 "T00:00:00"으로 끝나는 문자열이 아니라 반드시 JSON의 null이어야 합니다. 00:00은 "시각 모름"을 뜻하지 않으며 문자 그대로 자정을 의미하는데, 화자의 의도가 그런 경우는 거의 없습니다.
...

{
  ...
  "tasks": [
    {"title": "할 일 내용, 한국어로", "due_hint": "마감일의 원래 표현(없으면 null)", "due_date": "YYYY-MM-DD (추론할 수 없으면 null. recurrence를 사용하는 경우에도 null 가능)", "reminder_at": "YYYY-MM-DDTHH:mm:00 (명시적인 시각이 없으면 null)", "reminder_end_at": "YYYY-MM-DDTHH:mm:00 (명시적인 종료 시각이 없으면 null)", "relative_offset_minutes": 녹음 시점 기준 상대 시간("30분 후", "3시간 후" 등)일 때만 — 지금으로부터 몇 분 후인지를 나타내는 정수. 그 외에는 생략하거나 null, "span_days": 여러 날에 걸친 종일 일정일 때만 — 시작일을 포함한 총 일수를 나타내는 정수(예: "금요일부터 일요일까지" = 3. 종료 시각이 있고 마지막 날 밤이 자정을 넘기면 그 다음 날도 포함), "recurrence": {"weekdays": ["Mon","Tue","Wed","Thu","Fri","Sat","Sun" 중 반복되는 요일을 영어 3글자 약어 배열로], "start_date": "YYYY-MM-DD (반복 시작일)", "end_date": "YYYY-MM-DD (반복 종료일. 종료 시점이 전혀 언급되지 않았다면 직접 계산하지 말고 null로 두세요 — 앱 코드가 적절한 기본값을 채웁니다)", "interval_weeks": 1 이상의 정수, 기본값 1=매주. "격주"면 2} — 반복 패턴이 아닌 일반 할 일에서는 이 필드를 생략하거나 null로 둠, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sun 중 하나", "weeks_ahead": 0 이상의 정수 (0=가장 가까운 발생일, 1=그로부터 1주 후, 2=2주 후...), "days_before": 0 이상의 정수, 기본값 0. "그 전에" 관계로 다른 할 일의 마감을 빌려올 때만 1 이상 사용, "anchor_title": days_before가 1 이상일 때만 — 기준이 되는 일정 자체(예: "사촌 결혼식")가 화자 자신의 1인칭 행동 동사 없이 단순한 사실로만 서술되어 있어서, 이를 별도의 독립된 할 일로는 추출하지 않기로 했다면, 그 기준 일정을 짧은 명사구로 여기에 적으세요(예: "Cousin's wedding"). 기준 일정을 이미 별도의 할 일로 추출했다면 생략하거나 null로 둠.} — 마감일이 요일명을 중심으로 언급된 경우에만 사용하고, 그 외에는 생략하거나 null로 둠, "due_month": {"months_ahead": 정수, 이번 달을 0으로 한 몇 달 후인지(다음 달=1, 3개월 후=3), "day": 1-31의 정수, 화자가 구체적인 날짜를 언급한 경우에만(생략 시 오늘과 같은 날짜를 재사용)} — 마감일이 "다음 달"/"3개월 후"/"1일까지"처럼 월 단위의 상대적 표현인 경우에만 사용하고, 그 외에는 생략하거나 null로 둠}
  ],
  "notes": [
 ...
 * 同じ日にちを使う。dayが指定されておりmonths_ahead:0（＝月への明示的な
 * 言及が無い「直近の該当日」扱い）の場合のみ、その日にちが今月では既に
 * 過ぎていれば自動で来月へ繰り上げる（due_weekdayの「直近の該当日」と同じ
 * 考え方——モデルに「もう過ぎているか」を判断させない）。月末クランプ
 * （例:1/31の1ヶ月後は2/28）も併せて行う。 */
function resolveTaskDueMonth(
  task: StructuredResult["tasks"][number],
  todayDateStr: string
): StructuredResult["tasks"][number] {
  const dm = task.due_month;
  if (!dm || typeof dm.months_ahead !== "number" || !Number.isFinite(dm.months_ahead)) return task;

  const [y, m, d] = todayDateStr.split("-").map(Number);
  const monthsAheadRaw = Math.max(0, Math.round(dm.months_ahead));
  ...
  const rollover = hasExplicitDay && monthsAheadRaw === 0 && day < d ? 1 : 0;
...
function enforceAnchorDateConsistency(
  ...
  const fixedTasks = tasks.map((task) => {
    ...
    if (!hintLower) return task;

    const anchor = tasks.find((other) => {
      if (other === task) return false;
      const odw = other.due_weekday;
      if (!odw || (odw.days_before ?? 0) > 0) return false;
    ...
    if (!anchor?.due_weekday) return task;
    const anchorDw = anchor.due_weekday;
    if (anchorDw.day === dw.day && anchorDw.weeks_ahead === dw.weeks_ahead) return task;

    logger.warn("processVoiceMemo: forcing days_before task's day/weeks_ahead to match its anchor task", {
      taskTitle: task.title,
      anchorTitle: anchor.title,
      originalDueWeekday: dw,
      anchorDueWeekday: anchorDw,
    });

    return {
      ...task,
      due_weekday: { ...dw, day: anchorDw.day, weeks_ahead: anchorDw.weeks_ahead },
    };
  });
...
function toClientResponse(
  ...
  return {
    ...
    tasks: debugTasksAfterSpanEnd
      ...
      // （プロンプトでnullにするよう指示済みだが、それでも稀に発生する）。
      // 素直な深夜0時ちょうどの予定は現実的にほぼ無いため、この値は「時刻不明」
      // の誤りとみなしnullへ丸め、終日タスク（前日[allDayReminderHour]時通知）
      // へフォールバックさせる方が実害が少ない。
      const reminderAtRaw =
        task.reminder_at && isoDateTime.test(task.reminder_at) ? task.reminder_at : null;
      const reminderAt = reminderAtRaw && reminderAtRaw.endsWith("T00:00:00") ? null : reminderAtRaw;
