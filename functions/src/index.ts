...
3. 発言は以下の3種類のいずれかに分類してください。
   ...
   - 【notes category="アイデア"】: 未確定な思いつき・疑問・アイデア・検討事項——その未確定な提案そのものだけを指し、そこに至るまでの事実の説明・経緯は含めない(その経緯部分は別の感情ログのnoteにする。下記参照)。
...

{
  "summary": "全体の1行要約",
  "tasks": [
    {"title": "タスク内容", "due_hint": "期限の元の言い回し（なければnull）", "due_date": "YYYY-MM-DD（推測できなければnull。recurrenceを使う場合もnullでよい）", "reminder_at": "YYYY-MM-DDTHH:mm:00（時刻の明言が無ければnull）", "reminder_end_at": "YYYY-MM-DDTHH:mm:00（終了時刻の明言が無ければnull）", "recurrence": {"weekdays": ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"のうち繰り返す曜日を英語3文字表記で], "start_date": "YYYY-MM-DD（繰り返し開始日）", "end_date": "YYYY-MM-DD（繰り返し終了日）", "interval_weeks": 1以上の整数（省略時1=毎週。「隔週」等なら2）} … 繰り返しパターンでない通常のタスクではこのフィールド自体を省略するかnullにする, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sunのいずれか", "weeks_ahead": 0以上の整数（0=直近の該当日、1=その1週間後、2=その2週間後...）, "days_before": 0以上の整数（省略時0。別タスクの締切を「その前に」で借りる場合のみ1以上）} … 曜日名を軸に期限が語られた場合のみ使用し、それ以外は省略するかnullにする}
  ],
  "notes": [
...
3. Classify each utterance into exactly one of these three categories:
   ...
   - [notes category="アイデア"]: an unconfirmed idea, question, thought, or something to consider — only the hedged suggestion itself, not any factual narrative or backstory that led into it (that backstory belongs in its own 感情ログ note instead, see below).
...
3. Clasifica cada enunciado en exactamente una de estas tres categorías:
   ...
   - [notes category="アイデア"]: una idea, pregunta o pensamiento sin confirmar, o algo a considerar — solo la sugerencia sin confirmar en sí, no la narración/contexto fáctico que llevó a ella (ese contexto va en su propia nota 感情ログ, ver abajo).
...
3. Klassifiziere jede Äußerung in genau eine dieser drei Kategorien:
   ...
   - [notes category="アイデア"]: eine unbestätigte Idee, Frage oder ein Gedanke, oder etwas zum Nachdenken — nur der unbestätigte Vorschlag selbst, nicht die sachliche Erzählung/Vorgeschichte, die dazu geführt hat (diese gehört in eine eigene 感情ログ-Notiz, siehe unten).
...
3. 각 발화를 다음 세 카테고리 중 정확히 하나로 분류하세요:
   ...
   - [notes category="アイデア"]: 아직 확정되지 않은 아이디어, 질문, 생각, 또는 고려해볼 만한 것 — 그 미확정 제안 자체만을 가리키며, 거기에 이르기까지의 사실적 서술·배경은 포함하지 않습니다(그 배경은 별도의 感情ログ note가 됩니다. 아래 참고).
...

{
  ...
  "tasks": [
    {"title": "할 일 내용, 한국어로", "due_hint": "마감일의 원래 표현(없으면 null)", "due_date": "YYYY-MM-DD (추론할 수 없으면 null. recurrence를 사용하는 경우에도 null 가능)", "reminder_at": "YYYY-MM-DDTHH:mm:00 (명시적인 시각이 없으면 null)", "reminder_end_at": "YYYY-MM-DDTHH:mm:00 (명시적인 종료 시각이 없으면 null)", "recurrence": {"weekdays": ["Mon","Tue","Wed","Thu","Fri","Sat","Sun" 중 반복되는 요일을 영어 3글자 약어 배열로], "start_date": "YYYY-MM-DD (반복 시작일)", "end_date": "YYYY-MM-DD (반복 종료일)", "interval_weeks": 1 이상의 정수, 기본값 1=매주. "격주"면 2} — 반복 패턴이 아닌 일반 할 일에서는 이 필드를 생략하거나 null로 둠, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sun 중 하나", "weeks_ahead": 0 이상의 정수 (0=가장 가까운 발생일, 1=그로부터 1주 후, 2=2주 후...), "days_before": 0 이상의 정수, 기본값 0. "그 전에" 관계로 다른 할 일의 마감을 빌려올 때만 1 이상 사용} — 마감일이 요일명을 중심으로 언급된 경우에만 사용하고, 그 외에는 생략하거나 null로 둠}
  ],
  "notes": [
...
3. Classe chaque énoncé dans exactement une de ces trois catégories :
   ...
   - [notes category="アイデア"] : une idée, une question ou une pensée non confirmée, ou quelque chose à considérer — seulement la suggestion non confirmée elle-même, pas le récit/contexte factuel qui y a mené (ce contexte va dans sa propre note 感情ログ, voir ci-dessous).
