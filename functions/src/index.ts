...
3. 発言は以下の3種類のいずれかに分類してください。
   - 【tasks（ToDo）】: 「確定した行動」。話者がこれから実際にやる・やらないといけないと言っていること。**既に完了した行動はtasksにしないでください**——「〜した」「〜してきた」のように過去形・完了形で語られている行動(例:「今朝走ってきた」「マイクとコーヒー飲んできた」「実家に寄って荷物を渡してきた」)は、たとえ動詞が具体的な行動を表していても、それだけでタスクとして抽出しないこと。これらは既に終わった出来事の報告であり、下記の【notes category="感情ログ"】に分類すべき内容です。tasksに入れてよいのは、話者がまだやっていない・これからやる必要があると述べている行動だけです。
   ...
   - 【notes category="感情ログ"】: 日記エントリ。感情・気分・愚痴・モヤモヤ・出来事の振り返りだけでなく、**行動を伴わない単なる事実の出来事の報告**(誰と会った・何をした等)も含む。強い感情表現が無くても構いません——事実を淡々と述べただけの内容でも、感情語が無いという理由だけで取りこぼしたり近くのアイデア/タスクに紛れ込ませたりせず、こちらに分類してください。
...
タスクの期限が「その前に」「その後」のように、同じ書き起こし内の別のタスクを指すだけの相対的な言い方しか無い場合、その別のタスク自体に解決可能な日付(due_dateまたはdue_weekday)があれば、このタスクにも同じ基準日を持たせて、期限無しのままにしないでください。ただし「その前に」は基準日と同じ日ではなく**基準日の前日**を意味するので、due_weekdayを使う場合はweeks_ahead等をそのままコピーした上でdays_before:1を追加してください(「その後」のように前ではなく後ろを指す場合はdays_beforeを付けず基準日をそのまま使う)。借りられる基準日が本当に無い場合だけ、due_dateをnullのままにしてください。
...

{
  "summary": "全体の1行要約",
  "tasks": [
    {"title": "タスク内容", "due_hint": "期限の元の言い回し（なければnull）", "due_date": "YYYY-MM-DD（推測できなければnull。recurrenceを使う場合もnullでよい）", "reminder_at": "YYYY-MM-DDTHH:mm:00（時刻の明言が無ければnull）", "reminder_end_at": "YYYY-MM-DDTHH:mm:00（終了時刻の明言が無ければnull）", "recurrence": {"weekdays": ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"のうち繰り返す曜日を英語3文字表記で], "start_date": "YYYY-MM-DD（繰り返し開始日）", "end_date": "YYYY-MM-DD（繰り返し終了日。終了時期の言及が無ければ、自分で計算しようとせずnullにする——アプリ側のコードが妥当な既定値を補う）", "interval_weeks": 1以上の整数（省略時1=毎週。「隔週」等なら2）} … 繰り返しパターンでない通常のタスクではこのフィールド自体を省略するかnullにする, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sunのいずれか", "weeks_ahead": 0以上の整数（0=直近の該当日、1=その1週間後、2=その2週間後...）, "days_before": 0以上の整数（省略時0。別タスクの締切を「その前に」で借りる場合のみ1以上）} … 曜日名を軸に期限が語られた場合のみ使用し、それ以外は省略するかnullにする}
  ],
  "notes": [
...
3. Classify each utterance into exactly one of these three categories:
   ...
   - [notes category="感情ログ"]: a diary entry — a feeling, mood, complaint, reflection, OR simply a factual account of something that happened, with no associated action. Don't require strong emotional wording for this: a plain factual recap of an event (who the speaker saw, what they did) still qualifies and should default here, rather than being dropped or folded into a nearby idea/task just because it lacks an explicit feeling word.
...
3. Clasifica cada enunciado en exactamente una de estas tres categorías:
   ...
   - [notes category="感情ログ"]: una entrada de diario — un sentimiento, estado de ánimo, queja o reflexión, O simplemente un relato fáctico de algo que ocurrió, sin ninguna acción asociada. No exijas un lenguaje emocional fuerte para esto: un relato puramente fáctico de un evento (con quién estuvo, qué hizo) también cuenta y debe ir aquí por defecto, en lugar de perderse o fundirse en una idea/tarea cercana solo porque le falta una palabra de sentimiento explícita.
...
3. Klassifiziere jede Äußerung in genau eine dieser drei Kategorien:
   ...
   - [notes category="感情ログ"]: ein Tagebucheintrag — ein Gefühl, eine Stimmung, eine Beschwerde oder eine Reflexion, ODER einfach eine sachliche Schilderung von etwas Geschehenem, ohne zugehörige Handlung. Verlange dafür keine starke emotionale Formulierung: Eine rein sachliche Zusammenfassung eines Ereignisses (mit wem, was gemacht) zählt ebenfalls und sollte standardmäßig hierhin gehen, statt verloren zu gehen oder in eine nahegelegene Idee/Aufgabe eingemischt zu werden, nur weil ein explizites Gefühlswort fehlt.
...
3. 각 발화를 다음 세 카테고리 중 정확히 하나로 분류하세요:
   - [tasks (할 일)]: "확정된 행동" — 화자가 앞으로 하겠다고 말했거나 해야 한다고 말한 것. **이미 일어난 일은 절대로 할 일로 만들지 마세요.** "달리기를 했다", "마이크와 커피를 마셨다", "엄마 집에 들러 장 봐온 것을 전해줬다"처럼 과거형·완료형으로 이미 끝난 것으로 서술된 행동은, 구체적인 동사+목적어가 나오더라도 할 일이 아닙니다 — 이는 이미 끝난 일에 대한 보고이며, 아래의 [notes category="感情ログ"]에만 분류해야 합니다. 화자가 아직 하지 않았고 앞으로 해야 한다고 말하는 행동일 때만 할 일로 추출하세요.
   ...
   - [notes category="感情ログ"]: 일기 항목 — 감정·기분·불평·회고뿐 아니라, **관련된 행동이 없는 단순한 사실 사건 보고**도 포함합니다. 강한 감정 표현이 없어도 됩니다 — 감정 단어가 없다는 이유만으로 놓치거나 근처의 아이디어/할 일에 섞어 넣지 말고, 이쪽으로 분류하세요.
...
할 일의 유일한 마감 단서가 같은 녹취록 속 다른 할 일을 가리키는 상대적 표현뿐이고(예: "그 전에", "여행 가기 전에"), 그 다른 할 일 자체는 해결 가능한 날짜(due_date 또는 due_weekday)를 가지고 있다면, 이 할 일에도 그 기준 날짜를 부여해서 날짜 없이 남기지 마세요. 다만 "그 전에"는 기준 날짜와 같은 날이 아니라 **기준 날짜의 하루 전**을 뜻하므로, due_weekday를 쓸 경우 기준 할 일의 day/weeks_ahead를 그대로 복사한 뒤 days_before: 1을 추가하세요("그 후에"처럼 뒤를 가리키는 경우는 days_before 없이 기준 날짜를 그대로 사용). 빌려올 기준 날짜가 전혀 없을 때만 due_date를 null로 두세요.
...

{
  ...
  "tasks": [
    {"title": "할 일 내용, 한국어로", "due_hint": "마감일의 원래 표현(없으면 null)", "due_date": "YYYY-MM-DD (추론할 수 없으면 null. recurrence를 사용하는 경우에도 null 가능)", "reminder_at": "YYYY-MM-DDTHH:mm:00 (명시적인 시각이 없으면 null)", "reminder_end_at": "YYYY-MM-DDTHH:mm:00 (명시적인 종료 시각이 없으면 null)", "recurrence": {"weekdays": ["Mon","Tue","Wed","Thu","Fri","Sat","Sun" 중 반복되는 요일을 영어 3글자 약어 배열로], "start_date": "YYYY-MM-DD (반복 시작일)", "end_date": "YYYY-MM-DD (반복 종료일. 종료 시점이 전혀 언급되지 않았다면 직접 계산하지 말고 null로 두세요 — 앱 코드가 적절한 기본값을 채웁니다)", "interval_weeks": 1 이상의 정수, 기본값 1=매주. "격주"면 2} — 반복 패턴이 아닌 일반 할 일에서는 이 필드를 생략하거나 null로 둠, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sun 중 하나", "weeks_ahead": 0 이상의 정수 (0=가장 가까운 발생일, 1=그로부터 1주 후, 2=2주 후...), "days_before": 0 이상의 정수, 기본값 0. "그 전에" 관계로 다른 할 일의 마감을 빌려올 때만 1 이상 사용} — 마감일이 요일명을 중심으로 언급된 경우에만 사용하고, 그 외에는 생략하거나 null로 둠}
  ],
  "notes": [
...
3. Classe chaque énoncé dans exactement une de ces trois catégories :
   ...
   - [notes category="感情ログ"] : une entrée de journal — un sentiment, une humeur, une plainte ou une réflexion, OU simplement un récit factuel de quelque chose qui s'est passé, sans action associée. N'exige pas un langage émotionnel fort pour ça : un simple récapitulatif factuel d'un événement (qui la personne a vu, ce qu'elle a fait) compte aussi et doit aller ici par défaut, plutôt que d'être perdu ou fondu dans une idée/tâche voisine juste parce qu'il manque un mot de sentiment explicite.
