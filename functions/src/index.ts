...
interface StructuredResult {
  ...
  tasks: {
    ...
    recurrence?: {
      /** "monthly"を指定した場合のみ月次繰り返し（weekdaysではなく
       * day_of_monthを使う）。省略時は"weekly"（従来通りweekdaysベース）。 */
      type?: "weekly" | "monthly" | null;
      /** weekly（省略時含む）の場合のみ必須。monthlyでは無視される。 */
      weekdays?: string[];
       ...
       * デフォルトの窓（weeklyは30日分、monthlyは6ヶ月分）を自動で展開する
       * （due_weekdayの単一曜日版と同じ「終了時期不明でも生成し続ける」
       * 設計をmonthlyにも拡張したもの）。 */
