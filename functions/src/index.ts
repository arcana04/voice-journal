...
具体例:「今週じゃなくて来週末に実家に帰る予定だから、その前に車を点検に出しておかないと」は、必ず「実家に帰る」(due_weekdayで来週末の該当曜日、weeks_ahead:1)と「車を点検に出す」(基準タスクと同じdue_weekdayを持たせる——「その前に」がその締切を借りる形になる、due_hintは裸の「その前に」ではなく「実家に帰る前に」)の2件の別々のタスクになるべきです——どちらか一方だけになったり、借りられる基準日があるのに車の点検の方のdue_weekdayを省略したりしてはいけません。
...
タスクの期限が「その前に」「その後」のように、同じ書き起こし内の別のタスクを指すだけの相対的な言い方しか無い場合、その別のタスク自体に解決可能な日付(due_dateまたはdue_weekday)があれば、このタスクにも同じdue_weekday(またはdue_date)を持たせて、期限無しのままにしないでください——「その前に」は実質「その日までに」という締切を意味します。借りられる基準日が本当に無い場合だけ、due_dateをnullのままにしてください。
...
If a task's only due-date reference is a relative pointer to another task in the same transcript (e.g. "before then", "before the trip", "after that"), and that other task itself has a resolvable date (a due_date, or a due_weekday), give this task the SAME due_weekday (or due_date) as that anchor task too, so it gets an actual deadline instead of staying undated — "before then" effectively means "no later than that date". Only fall back to leaving due_date null when there is no such anchor date to borrow at all.
...
Si la única referencia de fecha límite de una tarea es un puntero relativo a otra tarea de la misma transcripción (p. ej. "antes de eso", "antes del viaje"), y esa otra tarea tiene a su vez una fecha resoluble (un due_date o un due_weekday), dale a esta tarea el MISMO due_weekday (o due_date) que esa tarea ancla, para que tenga una fecha límite real en vez de quedar sin fecha — "antes de eso" equivale en la práctica a "a más tardar ese día". Solo deja due_date en null cuando no haya ninguna fecha ancla que tomar prestada.
...
Konkretes Beispiel: "Nicht dieses Wochenende, aber nächstes Wochenende fahre ich zu meinen Eltern, also sollte ich vorher wohl das Auto checken lassen" MUSS genau zwei separate Aufgaben ergeben — "Zu den Eltern fahren" (due_weekday: Wochenend-Tag, weeks_ahead: 1) UND "Auto checken lassen" (dasselbe due_weekday wie die Bezugs-Aufgabe, da "vorher" sich diese Frist ausleiht, mit due_hint "vor dem Besuch bei den Eltern", nicht dem bloßen "davor") — niemals nur eine der beiden, und niemals ohne due_weekday bei der Auto-Aufgabe, obwohl ein Bezugsdatum zum Übernehmen vorhanden ist.
...
Wenn sich die einzige Fälligkeitsangabe einer Aufgabe relativ auf eine andere Aufgabe im selben Transkript bezieht (z. B. "davor", "vor der Reise"), und diese andere Aufgabe selbst ein auflösbares Datum hat (ein due_date oder ein due_weekday), gib dieser Aufgabe ebenfalls dasselbe due_weekday (oder due_date) wie der Bezugs-Aufgabe, damit sie eine echte Frist bekommt statt ohne Datum zu bleiben — "davor" bedeutet praktisch "spätestens an diesem Datum". Setze due_date nur dann auf null, wenn es gar kein solches Bezugsdatum zum Übernehmen gibt.
...
구체적인 예: "이번 주말 말고 다음 주말에 부모님 뵈러 갈 건데, 그 전에 차 점검을 받아야 할 것 같아"는 반드시 "부모님 뵈러 가기"(due_weekday: 주말 요일, weeks_ahead: 1) 그리고 "차 점검받기"(기준 할 일과 동일한 due_weekday를 부여 — "그 전에"가 그 마감을 빌려오는 형태, due_hint는 맨 "그 전에"가 아니라 "부모님 뵈러 가기 전에")라는 정확히 두 개의 별도 할 일이 되어야 합니다 — 둘 중 하나만 나오거나, 빌려올 기준 날짜가 있는데도 차 점검 쪽의 due_weekday를 생략해서는 안 됩니다.
...
할 일의 유일한 마감 단서가 같은 녹취록 속 다른 할 일을 가리키는 상대적 표현뿐이고(예: "그 전에", "여행 가기 전에"), 그 다른 할 일 자체는 해결 가능한 날짜(due_date 또는 due_weekday)를 가지고 있다면, 이 할 일에도 그 기준 할 일과 동일한 due_weekday(또는 due_date)를 부여해서 날짜 없이 남기지 마세요 — "그 전에"는 사실상 "그 날짜까지"라는 마감을 뜻합니다. 빌려올 기준 날짜가 전혀 없을 때만 due_date를 null로 두세요.
...

{
  ...
  "tasks": [
    {"title": "할 일 내용, 한국어로", "due_hint": "마감일의 원래 표현(없으면 null)", "due_date": "YYYY-MM-DD (추론할 수 없으면 null. recurrence를 사용하는 경우에도 null 가능)", "reminder_at": "YYYY-MM-DDTHH:mm:00 (명시적인 시각이 없으면 null)", "reminder_end_at": "YYYY-MM-DDTHH:mm:00 (명시적인 종료 시각이 없으면 null)", "recurrence": {"weekdays": ["Mon","Tue","Wed","Thu","Fri","Sat","Sun" 중 반복되는 요일을 영어 3글자 약어 배열로], "start_date": "YYYY-MM-DD (반복 시작일)", "end_date": "YYYY-MM-DD (반복 종료일)", "interval_weeks": 1 이상의 정수, 기본값 1=매주. "격주"면 2} — 반복 패턴이 아닌 일반 할 일에서는 이 필드를 생략하거나 null로 둠, "due_weekday": {"day": "Mon/Tue/Wed/Thu/Fri/Sat/Sun 중 하나", "weeks_ahead": 0 이상의 정수 (0=가장 가까운 발생일, 1=그로부터 1주 후, 2=2주 후...)} — 마감일이 요일명을 중심으로 언급된 경우에만 사용하고, 그 외에는 생략하거나 null로 둠}
  ],
  "notes": [
...
Exemple concret : « Pas ce week-end, mais le week-end prochain je vais voir mes parents, donc je devrais sans doute faire vérifier la voiture avant ça » DOIT produire exactement deux tâches séparées — « Aller voir mes parents » (due_weekday : jour du week-end, weeks_ahead : 1) ET « Faire vérifier la voiture » (la MÊME due_weekday que la tâche de référence, puisque « avant ça » emprunte cette échéance, avec due_hint « avant d'aller voir mes parents », pas le simple « avant ça ») — jamais une seule des deux, et jamais en omettant la due_weekday de la tâche voiture alors qu'une date de référence existe à emprunter.
...
Si la seule référence de date limite d'une tâche est un renvoi relatif à une autre tâche de la même transcription (par ex. « avant ça », « avant le voyage »), et que cette autre tâche a elle-même une date résoluble (une due_date ou une due_weekday), donne à cette tâche la MÊME due_weekday (ou due_date) que cette tâche de référence, pour qu'elle ait une vraie échéance plutôt que de rester sans date — « avant ça » signifie en pratique « au plus tard à cette date ». Ne laisse due_date à null que lorsqu'il n'y a vraiment aucune date de référence à emprunter.
