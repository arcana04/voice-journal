...
具体例:「今週じゃなくて来週末に実家に帰る予定だから、その前に車を点検に出しておかないと」は、必ず「実家に帰る」(来週末に解決)と「車を点検に出す」(due_dateはnullのまま、due_hintは「その前に」)の2件の別々のタスクになるべきです——どちらか一方だけになったり、来週末の日付が車の点検の方にもコピーされたりしてはいけません。
...
Concrete example: "Not this weekend, but next weekend I'm driving up to see my parents, so I should probably get the car checked out before then" MUST produce exactly two separate tasks — "Drive up to see parents" (due_weekday resolved to next weekend) AND "Get the car checked" (due_date left null, due_hint "before then") — never just one of the two, and never with next weekend's date copied onto the car-check task.
...
Ejemplo concreto: "No este fin de semana, sino el próximo, voy a ir a visitar a mis padres, así que probablemente debería revisar el coche antes de eso" DEBE producir exactamente dos tareas separadas — "Ir a visitar a mis padres" (due_weekday resuelto al próximo fin de semana) Y "Revisar el coche" (due_date en null, due_hint "antes de eso") — nunca solo una de las dos, y nunca con la fecha del próximo fin de semana copiada a la tarea de revisar el coche.
...
Konkretes Beispiel: "Nicht dieses Wochenende, aber nächstes Wochenende fahre ich zu meinen Eltern, also sollte ich vorher wohl das Auto checken lassen" MUSS genau zwei separate Aufgaben ergeben — "Zu den Eltern fahren" (due_weekday auf nächstes Wochenende aufgelöst) UND "Auto checken lassen" (due_date bleibt null, due_hint "vorher") — niemals nur eine der beiden, und niemals mit dem Datum von nächstes Wochenende auf die Auto-Aufgabe kopiert.
...
구체적인 예: "이번 주말 말고 다음 주말에 부모님 뵈러 갈 건데, 그 전에 차 점검을 받아야 할 것 같아"는 반드시 "부모님 뵈러 가기"(due_weekday를 다음 주말로 해결) 그리고 "차 점검받기"(due_date는 null로 두고, due_hint는 "그 전에")라는 정확히 두 개의 별도 할 일이 되어야 합니다 — 둘 중 하나만 나오거나, 다음 주말 날짜가 차 점검 쪽에도 복사되어서는 안 됩니다.
...
Exemple concret : « Pas ce week-end, mais le week-end prochain je vais voir mes parents, donc je devrais sans doute faire vérifier la voiture avant ça » DOIT produire exactement deux tâches séparées — « Aller voir mes parents » (due_weekday résolu au week-end prochain) ET « Faire vérifier la voiture » (due_date laissé à null, due_hint « avant ça ») — jamais une seule des deux, et jamais avec la date du week-end prochain recopiée sur la tâche de la voiture.
